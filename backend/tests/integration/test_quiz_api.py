...
async def test_get_quiz_success(client, db_session):
    ...
    quiz = Quiz(teacher_id=SIMPLE_TEACHER_ID, title="Seed Quiz", status=QuizStatus.DRAFT)
    db_session.add(quiz)
    await db_session.commit()
    await db_session.refresh(quiz)
    
    response = await client.get(f"/quizzes/{quiz.id}")
...
async def test_get_quiz_bola(client, db_session):
    ...
    other_teacher = uuid4()
    user = User(id=other_teacher, email="other@example.com", full_name="Other Teacher")
    db_session.add(user)
    await db_session.flush()
    
    # Seed quiz owned by OTHER
    quiz = Quiz(teacher_id=other_teacher, title="Other Quiz", status=QuizStatus.DRAFT)
    db_session.add(quiz)
    await db_session.commit()
...
async def test_update_quiz(client, db_session):
    from app.core.deps import SIMPLE_TEACHER_ID
    quiz = Quiz(teacher_id=SIMPLE_TEACHER_ID, title="To Update", status=QuizStatus.DRAFT)
    db_session.add(quiz)
    await db_session.commit()
    
    payload = {"title": "Updated Title"}
    response = await client.put(f"/quizzes/{quiz.id}", json=payload)
    assert response.status_code == 200
