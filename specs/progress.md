...
- **Ticket**: QQ-BUG-003 - Fix missing Edit access (JWT sub/id mismatch)
- **Outcome**: Mapped JWT `sub` claim to user `id` in `AuthContext` to ensure frontend ownership checks (`quiz.owner_id === user.id`) work correctly.
- **Artifacts**: `frontend/src/features/auth/context/AuthContext.tsx`, `specs/features/quiz-management/plan_QQ-BUG-003.md`.
- **Verification**: Ownership check now correctly identifies the logged-in teacher as the editor of their own quizzes.
